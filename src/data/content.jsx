...
export const PROJECTS = [
  defineProject({
    ...
    target: '採用担当の方や、制作を依頼するか検討している方。限られた時間で「何を作れる人なのか」を掴みたい相手を想定しています。',
    purpose: '制作物が公開先ごとにばらばらで、まとめて見てもらう手段がありませんでした。作品を一覧できる場所を用意し、興味を持った人がダウンロードや問い合わせまで迷わず進めることを目的にしています。',
    result: 'Web・アプリ・3Dにまたがる制作物をカテゴリで整理し、各詳細から実際のサイトへ直接移動できるようにしました。白基調の余白設計と3Dのヒーロー表現によって、扱える技術と見せ方の両方を1つのサイトで示せる状態になっています。',
  }),
  defineProject({
    id: 'kakeibo-app',
    name: '便利な家計簿！',
    ...
    target: '日々の出費だけでなく、サブスクのような毎月の固定費まで把握しきれていない人。',
    purpose: '家計簿が続かない原因は入力の面倒さにあると考えました。記録の手間を減らしたうえで、収入・支出・サブスクを1か所で管理できる状態にすることを目的にしています。',
    result: '家計簿の作成・編集・完了管理を実装し、スマホでの利用を前提にレイアウトを設計しました。Flutterで組んだことで、同じUIをWebとアプリの両方に展開できています。',
  }),
  defineProject({
    id: 'desk-model',
    name: '3Dデスク周りモデル',
    ...
    target: '3D表現やゲーミング環境のビジュアルに関心がある人。',
    purpose: 'Blenderを身につけるにあたって、自分が好きなゲーム周りを題材にすれば細部まで粘れると考え、練習用の題材として選びました。',
    result: 'デスクとモニターのモデリングから質感やライティングの調整まで一通り自分で組み立て、Blenderの制作フローを最後まで通せるようになりました。',
  }),
  defineProject({
    id: 'kyounomeigen-web',
    name: '今日の名言.web',
    ...
    features: ['API利用'],
    target: '毎日ひとつ、気分の切り替えになる言葉に触れたい人。',
    purpose: '思いつきから始めた企画ですが、外部APIからデータを取得して画面に出すまでの流れを、手を動かして身につけることも狙いにしていました。',
    result: '偉人やアニメ・漫画の名言を1日1件だけ表示する仕組みを実装し、API連携からFirebaseでの公開までを一人で完結させました。',
  }),
  defineProject({
    id: 'jigyomemo-web',
    name: 'じぎょメモ',
    ...
    target: '事業アイデアを思いついても、形にする手前で止まってしまう人。',
    purpose: '浮かんだアイデアがメモのまま埋もれてしまうのを防ぐため、項目に沿って埋めるだけで事業計画の体裁までまとまることを目的にしました。',
    result: 'アイデアのレベル感に応じてフレームワークを提示する構成にし、何を書けばいいか迷わず進められるUIにまとめました。',
  }),
];
